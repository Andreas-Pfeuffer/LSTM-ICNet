...
######################################################################################################################
###                                                                                                                ###
### Version 6: add a convLSTM at the end of each of the three branches and before softmax layer                    ###
###                                                                                                                ###
######################################################################################################################


class Faster_LSTM_ICNet_v6(Network):

    # Note, that depthwise (seperable) convLSTMs are not supported due to dimension problems
    ...
    VARIANTS = ["end2end",
                ...
                "freezeBranches",
                "freezeBranches_half",
                "freezeBranches_third",
                "freezeBranches_quarter",
                "spatial_end2end",
                "spatial_end2end_half",
                "spatial_end2end_third",
                "spatial_end2end_quarter",
                "spatial_freezeBranches",
                "spatial_freezeBrances_half",
                "spatial_freezeBranches_third",
                "spatial_freezeBranches_quarter",
                "depthwise_end2end",
                "depthwise_end2end_half",
                "depthwise_end2end_third",
                "depthwise_end2end_quarter",
                "depthwise_freezeBranches",
                "depthwise_freezeBrances_half",
                "depthwise_freezeBranches_third",
                "depthwise_freezeBranches_quarter",
                "separable_end2end",
                "separable_end2end_half",
                "separable_end2end_third",
                "separable_end2end_quarter",
                "separable_freezeBranches",
                "separable_freezeBrances_half",
                "separable_freezeBranches_third",
                "separable_freezeBranches_quarter"]
    ...
    def setup(self, is_training, num_classes, evaluation, timeSequence=1, variant=None):
        ...
        if variant == "freezeBranches" or variant == "spatial_freezeBranches" or variant == "freezeBranches_half" or variant == "spatial_freezeBranches_half" or variant == "freezeBranches_third" or variant == "spatial_freezeBranches_third" or variant == "freezeBranches_quarter" or variant == "spatial_freezeBranches_quarter" or variant == "depthwise_freezeBranches" or variant == "depthwise_freezeBranches_half" or variant == "depthwise_freezeBranches_third" or variant == "depthwise_freezeBranches_quarter" or variant == "separable_freezeBranches" or variant == "separable_freezeBranches_half" or variant == "separable_freezeBranches_third" or variant == "separable_freezeBranches_quarter":
        ...
        if variant == "spatial_end2end" or variant == "spatial_freezeBranches" or variant == "spatial_end2end_half" or variant == "spatial_freezeBranches_half" or variant == "spatial_end2end_third" or variant == "spatial_freezeBranches_third" or variant == "spatial_end2end_quarter" or variant == "spatial_freezeBranches_quarter":
            convLSTM_type = "spatial"
        elif variant == "depthwise_end2end" or variant == "depthwise_freezeBranches" or variant == "depthwise_end2end_half" or variant == "depthwise_freezeBranches_half" or variant == "depthwise_end2end_third" or variant == "depthwise_freezeBranches_third" or variant == "depthwise_end2end_quarter" or variant == "depthwise_freezeBranches_quarter":
            convLSTM_type = "depthwise"
        elif variant == "separable_end2end" or variant == "separable_freezeBranches" or variant == "separable_end2end_half" or variant == "separable_freezeBranches_half" or variant == "separable_end2end_third" or variant == "separable_freezeBranches_third" or variant == "separable_end2end_quarter" or variant == "separable_freezeBranches_quarter":
            convLSTM_type = "separable"
        else:
            convLSTM_type = "convolution" 

        # select channel size of convLSTM

        if variant == "end2end_half" or variant == "freezeBranches_half" or variant == "spatial_end2end_half" or variant == "spatial_freezeBrances_half" or variant == "depthwise_end2end_half" or variant == "depthwise_freezeBrances_half" or variant == "separable_end2end_half" or variant == "separable_freezeBrances_half":
            convLSTM_channelSize = 64
        elif variant == "end2end_third" or variant == "freezeBranches_third" or variant == "spatial_end2end_third" or variant == "spatial_freezeBranches_third" or variant == "depthwise_end2end_third" or variant == "depthwise_freezeBrances_third" or variant == "separable_end2end_third" or variant == "separable_freezeBrances_third":
            convLSTM_channelSize = 48
        elif variant == "end2end_quarter" or variant == "freezeBranches_quarter" or variant == "spatial_freezeBranches_third" or variant == "spatial_freezeBranches_quarter" or variant == "depthwise_end2end_quarter" or variant == "depthwise_freezeBrances_quarter" or variant == "separable_end2end_quarter" or variant == "separable_freezeBrances_quarter":
            convLSTM_channelSize = 32
        else: # use half channel size as default
            convLSTM_channelSize = 64
        ...
        # branch1: add convLSTM here and the skip

        if (convLSTM_type == "depthwise") or (convLSTM_type == "separable"):
            (self.feed('conv_sub4')
                 .conv(1, 1, convLSTM_channelSize, 1, 1, biased=False, relu=False, name='convlstm5_4_sub4_input_reduction')
                 .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm5_4_sub4', convtype=convLSTM_type))
        else:
            (self.feed('conv_sub4')
                 .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm5_4_sub4', convtype=convLSTM_type))

        (self.feed('conv_sub4')
             .conv(1, 1, 128-convLSTM_channelSize, 1, 1, biased=False, relu=False, name='convlstm5_4_sub4_1x1_reduce'))

        (self.feed('convlstm5_4_sub4',
                   'convlstm5_4_sub4_1x1_reduce')
             .concat(axis=-1, name='convlstm_b1_concat')
             .batch_normalization(relu=False, name='conv_sub4_bn'))

        # branch1: add convLSTM here and the skip - end

        # branch2: add convLSTM here and the skip

        (self.feed('conv3_1/relu')
                 .conv(1, 1, 128, 1, 1, biased=False, relu=False, name='conv3_1_sub2_proj'))

        if (convLSTM_type == "depthwise") or (convLSTM_type == "separable"):
            (self.feed('conv3_1_sub2_proj')
                .conv(1, 1, convLSTM_channelSize, 1, 1, biased=False, relu=False, name='convlstm3_1_sub2_input_reduction')
                .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm3_1_sub2', convtype=convLSTM_type))
        else:
            (self.feed('conv3_1_sub2_proj')
                .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm3_1_sub2', convtype=convLSTM_type))

        (self.feed('conv3_1_sub2_proj')
             .conv(1, 1, 128-convLSTM_channelSize, 1, 1, biased=False, relu=False, name='conv3_1_sub2_proj_1x1_reduce'))

        (self.feed('convlstm3_1_sub2',
                   'conv3_1_sub2_proj_1x1_reduce')
             .concat(axis=-1, name='convlstm_b2_concat')
             .batch_normalization(relu=False, name='conv3_1_sub2_proj_bn'))

        # branch2: add convLSTM here and the skip - end
        ...
        # branch3: add convLSTM here and the skip
        
        if (convLSTM_type == "depthwise") or (convLSTM_type == "separable"):
            (self.feed('conv3_sub1_proj')
                 .conv(1, 1, convLSTM_channelSize, 1, 1, biased=False, relu=False, name='convlstm3_sub1_input_reduction')
	             .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm3_sub1', convtype=convLSTM_type))
        else:
            (self.feed('conv3_sub1_proj')
	             .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm3_sub1', convtype=convLSTM_type))
        
        (self.feed('conv3_sub1_proj')
             .conv(1, 1, 128-convLSTM_channelSize, 1, 1, biased=False, relu=False, name='conv3_sub1_proj_1x1_reduce'))    

        (self.feed('convlstm3_sub1',
                   'conv3_sub1_proj_1x1_reduce')
             .concat(axis=-1, name='convlstm_b3_concat')
             .batch_normalization(relu=False, name='conv3_sub1_proj_bn'))

        # branch3: add convLSTM here and the skip - end
        ...
        # softmax: add convLSTM here and the skip
        
        if (convLSTM_type == "depthwise") or (convLSTM_type == "separable"):
            (self.feed('sub12_sum/relu')
                 .conv(1, 1, convLSTM_channelSize, 1, 1, biased=False, relu=False, name='convlstm6_cls_input_reduction')
                 .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm6_cls', convtype=convLSTM_type))
        else:
            (self.feed('sub12_sum/relu')
                 .convlstm(timeSequence,3,3, convLSTM_channelSize, name='convlstm6_cls', convtype=convLSTM_type))
        
        (self.feed('sub12_sum/relu')
             .conv(1, 1, 128-convLSTM_channelSize, 1, 1, biased=False, relu=False, name='conv_sub12_sum_1x1_reduce'))    

        (self.feed('convlstm6_cls',
                   'conv_sub12_sum_1x1_reduce')
             .concat(axis=-1, name='convlstm_softmax_concat')
        ...
        # softmax: add convLSTM here and the skip - end
